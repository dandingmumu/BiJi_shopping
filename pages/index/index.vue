			...
			<view class="shopping-menus__dropdown" :class="handleVanStickyShow || 'shopping-menusChange'">
				<van-sticky @change="handleVanSticky">
					<van-dropdown-menu class="shopping-menus__dropdownItem">
							...
							<van-row gutter="20">
								<van-col span="12">
									<view class="shopping-menus__dropdownHeight cityList">
										...
										<view class="cityListItem" :class="adCode == item.citycode ? 'on' : ''"
											...
											@click="handleCityClick(item.citycode,item.name)">
											{{ item.name }}
										</view>
									</view>
								</van-col>
								<van-col span="12">
									<view v-if="adCode == 0" class="shopping-menus__dropdownHeight">
										<view class="cityListItem" :class="cityNames == '全部区域' ? 'on' : ''"
											@click="handleCityClick(0,'不限')">
											不限
										</view>
										<view class="cityListItem" :class="cityNames == item.text ? 'on' : ''"
											v-for="(item, index) in cityClassifyListMeterData" :key="index"
											@click="handleCityMeterClick(item.meter,item.text)">
											{{ item.text }}
										</view>
									</view>
								</van-col>
						...
						<van-dropdown-item :title="categoryname" ref="categoryDropdownItem"
							class="shopping-menus__dropdownItem">
							<van-row gutter="20">
								<van-col span="12">
									<view class="shopping-menus__dropdownHeight cityList">
										<view class="cityListItem" :class="categoryactive == 0 ? 'on' : ''"
											@click="handlecategoryClick(0)">
											全部分类
										</view>
										<view class="cityListItem" :class="categoryactive == item.id ? 'on' : ''"
											v-for="(item, index) in categoryOptions" :key="index"
								...
								<van-col span="12">
									<view v-if="categoryactive == 0" class="shopping-menus__dropdownHeight">

									</view>
									<view v-else class='shopping-menus__dropdownHeight'>
										<view class="cityListItem " v-for="(item, index) in categoryChildList"
											:key="index" :class="categoryactive1 == item.id ? 'on' : ''"
											@click="handlecategorynameClick(item.cate_name,item.id)">
											{{ item.cate_name }}
										</view>
									</view>
								</van-col>
...
<script>
	...
	export default {
		...
		methods: {
			...
			/**
			 * 处理选择城市更新选择状态以及更新商家列表数据
			 * @param {Object} categoryactive
			 */
			handlecategoryClick(categoryactive, ) {
				this.categoryactive = categoryactive
				this.categoryOptions.map((item, index) => {
					if (item.id == categoryactive) {
						this.categoryChildList = item.children
					}
				})
				if (categoryactive == 0) {
					this.categoryactive1 = 0
					this.categoryname = '全部分类'
					this.$refs.categoryDropdownItem.toggle();
					this.searchList()
					return
				} else {
			...
			handlecategorynameClick(name, id) {
				this.categoryname = name
				this.categoryactive1 = id
				this.$refs.categoryDropdownItem.toggle();
				this.searchList()
			},
			searchList() {
				...
				const data = {
					cid: this.categoryactive1 != 0 ? this.categoryactive1 : 0,
					codes: this.adCode,
					recommend: this.recommendValue,
					mi: this.mi,
					location:this.location
				}
				this.productWhere.page = 1
